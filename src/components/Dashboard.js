...
const Dashboard = () => {
  ...
  useEffect(() => {
    axios
      .get("https://banner-takeuforward-backend.onrender.com/api/banner")
      .then((response) => {
        if (response && response.data) {
          ...
          console.log(
            "the value of the description link timer is: ",
            bannerData
          );
        }
      });
  ...
  const handleSave = () => {
    ...
    if (!description || description == "" || !timer || timer == "") {
      ...
      axios
        .post("https://banner-takeuforward-backend.onrender.com/api/banner", {
          description,
          link,
          timer,
        })
  ...
  return (
    <>
      ...
      <Box sx={{ padding: "20px", width: "50%", margin: "auto" }}>
        ...
        />{" "}
        <Button
          sx={{ width: "50%", margin: "1rem auto", display: "block" }}
          variant="contained"
          onClick={handleSave}
        >
          Save{" "}
